...
Clean, simple version that only uses Zoho Commerce API
"""
...
def get_zoho_commerce_products():
    ...
    try:
        ...
        if not access_token:
            logger.warning("⚠️ Zoho Commerce access token not configured")
            return []

        # Clean up the token
        access_token = access_token.strip()
        logger.info(f"🔑 Using Zoho access token: {access_token[:20]}...")
        ...
        logger.info(f"🌐 Calling Zoho API: {api_url}")
...
def find_product_url(product_name):
    """
    Find the actual product URL using Zoho Commerce API
    """
    ...
    try:
        ...
        # Get all products from Zoho Commerce API
        products = get_zoho_commerce_products()
        
        if not products:
            logger.warning("⚠️ No products available from Zoho Commerce API")
            return None
        
        logger.info(f"📦 Searching through {len(products)} products")
        
        # Use AI to find the best match
        best_match = analyze_products_with_ai(product_name, products)
        
        if best_match:
            logger.info(f"✅ Found match: {best_match['name']} -> {best_match['url']}")
            return best_match['url']
        
        logger.warning(f"⚠️ No product match found for '{product_name}'")
        return None
